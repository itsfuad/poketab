...
let scrolling = false;
let lastPageLength = document.querySelector('#messages').scrollTop;
let scroll = 0;
...
socket.on('updateUserList', function (users, ids, key, avatars) {
  ...
  for (let i = 0; i < users.length; i++) {
    if (users[i] == myname) {
      ul.insertAdjacentHTML('afterbegin', `<li class='user' id='${ids[i]}'><img height='30px' width='30px' src='/images/avatars/${avatars[i]}(custom).png'> ${users[i]} (You)</li>`);
      //ul.prepend($(`<li class='user' id='${ids[i]}'></li>`).html(`<img height='30px' width='30px' src='/images/avatars/${avatars[i]}(custom).png'> ${users[i]} (You)`));
    }else{
      ul.insertAdjacentHTML('beforeend', `<li class='user' id='${ids[i]}'><img height='30px' width='30px' src='/images/avatars/${avatars[i]}(custom).png'> ${users[i]} (You)</li>`);
      //ul.append($(`<li class='user' id='${ids[i]}'></li>`).html(`<img height='30px' width='30px' src='/images/avatars/${avatars[i]}(custom).png'> ${users[i]}`));
    }
  }
  ...
  //$('.keyname1').text(`${key}`);
  document.querySelector('.users').innerHTML = ul.innerHTML;
  //$('.users').html(ul);
});
...
socket.on('newMessage', function (message, sender_id, avatar, isReply, replyTo, replyText, id, targetId) {
  ...
  if (document.querySelector('#messages').lastElementChild.dataset.uid == sender_id && !isReply) {
    ...
    document.querySelector(`.${target} .avatar`).lastElementChild.style.visibility = 'hidden';
...
socket.on('messageSent', function (replaceId, id) {
  ...
  try{
    let msg = document.getElementById(replaceId);
    msg.setAttribute('id', id);
    msg.setAttribute('data-sent', 'true');
    msg.querySelector('.object').setAttribute('data-sent', 'true');
    msg.querySelector('.sent').remove();
  }
  catch(e){
    console.log(e);
  }
...
socket.on('deleteMessage', (messageId, user) => {
  try{
    ...
    let target = document.querySelector(`#${messageId} .object`);
    ...
    target.style.background = '#00000000';
    target.style.color = '#7d858c';
    ...
    document.querySelector(`[data-repid='${messageId}']`).style.background = '#000000c4';
    document.querySelector(`[data-repid='${messageId}']`).style.color = '#7d858c';
    if (user == myname) {
      popupMessage(`You deleted a message`);
      //$(`[data-repid='${messageId}']`).text(`You deleted this message`);
      document.querySelector(`[data-repid='${messageId}']`).innerText = `You deleted this message`;
    }else{
      popupMessage(`${user} deleted a message`);
      //$(`[data-repid='${messageId}']`).text(`${user} deleted this message`);
      document.querySelector(`[data-repid='${messageId}']`).innerText = `${user} deleted this message`;
    }
    updateScroll();
  }catch(e){
    console.log(e);
...
function addReact(target, userName, avatar, react){
  ...
  //$(`#${target} .reactor ul`).find(`li:contains(${user})`)

  if (document.querySelector(`#${target} .reactor ul`).querySelector(`li:contains(${user})`)) {
    //console.log($(`#${target} .reactor ul .react-or-${user}`).text());
    if (document.querySelector(`#${target} .reactor ul .react-or-${user} .emoticon`).innerText == emoji){
      //console.log('already reacted');
      socket.emit('removeReact', userName, target);
      return;
    }else{
      //$(`#${target} .reactor ul`).find(`li:contains(${user})`).remove();
      document.querySelector(`#${target} .reactor ul`).querySelector(`li:contains(${user})`).remove();
      ...
      if (userName == myname){
        ...
        document.querySelector(`#${target} .reactor ul`).insertAdjacentHTML('afterbegin', data);
      }else{
        //$(`#${target} .reactor ul`).append(data);
        document.querySelector(`#${target} .reactor ul`).insertAdjacentHTML('beforeend', data);
      }
    }
  }
  ...
  document.getElementById(target).querySelector('.object').style.marginBottom = '10px';
  updateScroll();
}
...
function reactBubble(id){
  ...
  let reactSrc = Array.from(document.querySelector(`#${id} .reactor ul`).children);
  let array = [];
  ...
  //$(`#${id} .reactions`).empty();
  document.querySelector(`#${id} .reactions`).innerHTML = '';
  ...
  for (let [key, value] of r_map) {
    if (count >= 3){
      ...
      document.querySelector(`#${id} .reactions li:last`).remove();
    }
    //$(`#${id} .reactions`).append(`<li class='emo'>${key}${value} </li>`);
    document.querySelector(`#${id} .reactions`).insertAdjacentHTML('beforeend', `<li class='emo'>${key}${value} </li>`);
    count++;
  }
}
...
function removeReaction(u_name, id){
  ...
  document.querySelector(`#${id} .reactions .${u_name == myname? 'You': u_name}`).remove();
  //$(`#${id} .react-or-${u_name == myname? 'You': u_name}`).remove();
  document.querySelector(`#${id} .react-or-${u_name == myname? 'You': u_name}`).remove();
 ...
 //   $(`#${id} .object`).css('margin-bottom', '');
  //}
  if (document.querySelector(`#${id} .reactions`).children.length == 0){
    document.querySelector(`#${id} .object`).style.marginBottom = '';
  }
...
function updateScroll(avatar = null, text = '') {
  if (scrolling) {
    if (text.length > 0) {
      ...
      document.querySelector('.newmessagepopup').style.display = 'block';
    }
    return;
  }
  setTimeout(() => {
    ...
    lastPageLength = element.scrollTop;
...
function unbindClicks(){
  document.querySelector('.reactionContainer').removeEventListener('click', clickOptionHide);
...
$('#messages').on('scroll', (event) => {
  scroll = document.querySelector('#messages').scrollTop;
...
$('#message-form').on('submit', function (e) {
  e.preventDefault();
  let messageTextbox = document.getElementById('textbox');
  let text = messageTextbox.value;
  ...
  if (isReply) {
    ...
    let msg = document.querySelector('#messages .message:last-child');
    //let msg = $('#messages ._message:last-child')[0];
...
document.getElementById('send-location').addEventListener('click', function () {
  ...
  let loc = document.getElementById('send-location');
  loc.setAttribute('disabled', 'disabled');
  loc.innerHTML = '<i class="fa-solid fa-location-crosshairs"></i>';
...
$("textarea").attr("style", "height:" + (this.scrollHeight) + "px;overflow-y:hidden;");
